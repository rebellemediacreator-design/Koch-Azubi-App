...
(() => {
  ...
  // ========================================================================
  // PRÜFUNG (EXAM) - ENHANCED FOR EACH YEAR
  // ========================================================================
  const exam = (() => {
    const els = {
    ...
    const buildPool = (y) => {
      return getGlossaryPool().filter(it => (it.years||[]).includes(Number(y)));
    };
    ...
    const tick = () => {
      ...
      const secs = s % 60;
      if (els.timer) els.timer.textContent = `${mins}:${secs.toString().padStart(2,"0")}`;
    };
    ...
    const render = () => {
      ...
      if (els.home) els.home.style.display = "none";
      if (els.name) els.name.textContent = `Prüfung LJ${currentYear}`;
      if (els.progress) els.progress.textContent = `${i+1}/${deck.length}`;
      if (els.meta) els.meta.textContent = "";
      if (els.text) els.text.textContent = q.term;
      if (els.choices) {
    ...
    const finish = () => {
      ...
      if (els.resultTitle) els.resultTitle.textContent = passed ? "✅ Bestanden!" : "❌ Nicht bestanden";
      if (els.resultMeta) els.resultMeta.textContent = `Ergebnis: ${score}/${deck.length} (${percent}%) · Zeit: ${els.timer?.textContent || "-"}`;
      if (els.wrongList) {
    ...
    const start = (year, onlyWrong=false) => {
      ...
      deck = base.sort(()=>Math.random()-0.5).slice(0, Math.min(questionCount, base.length)).map(makeQuestion);
    ...
    if (els.retryWrong) els.retryWrong.addEventListener("click", ()=> {
      ...
      start(currentYear, true);
    });
    if (els.backHome) els.backHome.addEventListener("click", ()=> { reset(false); setTab("start"); });
    if (els.choices) {
      els.choices.addEventListener("click", (e)=> {
        const b = e.target.closest("button.choice");
        if (!b || b.disabled) return;
